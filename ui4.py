...
class UI4:
    def __init__(self, game):
        ...
        self.level_num = self.mm.get_level_num() # 关卡背景图资源初始化
        self.sufs = [pygame.Surface((200, 400)) for _ in range(self.level_num)] # [None] * self.level_num
        ...
        texts = ['关卡' + str(i + 1) for i in range(self.level_num)] # 关卡文字
        self.level_texts = [font1.render(text, True, (0, 0, 0), None) for text in texts]

        self.update_progress() # 初始化用户进度、对勾图像
        self.finished_icon = pygame.image.load(game.resource_path('res/pic/finished_icon.png'))

        self.slider_back = pygame.image.load(game.resource_path('res/pic/slider_back.png')) # 初始化滑块资源
    ...
    def update(self, press_pos=False, mouse_wheel=False):
        ...
        if self.slider_rect.collidepoint(press_pos):
            ...
            da = (self.slider_loc[0] - self.slider_init_loc[0]) / (self.slider_max_loc[0] - self.slider_init_loc[0]) * (
                        300 * self.level_num - 900)
